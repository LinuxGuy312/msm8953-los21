...
ARCH=arm64 scripts/kconfig/merge_config.sh -O "out_$DEVICENAME" arch/arm64/configs/msm8953-perf_defconfig arch/arm64/configs/xiaomi/xiaomi.config arch/arm64/configs/xiaomi/sakura.config arch/arm64/configs/xiaomi/daisy.config lineageos_xx_append no_vdso
elif [ "$DEVICENAME" != "daikura" ]; then
	if ! [ -f arch/arm64/configs/xiaomi/"$DEVICENAME".config ]; then
  	echo arch/arm64/configs/xiaomi/"$DEVICENAME".config doesnt exist
  	exit
	fi
ARCH=arm64 scripts/kconfig/merge_config.sh -O "out_$DEVICENAME" arch/arm64/configs/msm8953-perf_defconfig arch/arm64/configs/xiaomi/xiaomi.config arch/arm64/configs/xiaomi/"$DEVICENAME".config lineageos_xx_append no_vdso
fi
...
make -j24 ARCH=arm64 SUBARCH=arm64 O=out_$DEVICENAME \
	...
	KBUILD_BUILD_FEATURES="ksu:# $KSUVER / cpu: 2208MHz 🔥 990mV 🧊 / gpu: 725MHz 🔥 / ddr: 1066MHz 🔥 / source: https://github.com/backslashxx/msm8953-los21 //"
